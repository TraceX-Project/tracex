...
const DeviceStackDetails = ({ device }: Props) => {
  ...
  const vlans = Array.from(
    new Map(device.devices.flatMap((d) => d.deviceVlans).map((vlan) => [vlan.id, vlan])).values()
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <DeviceFrontPanel src={device.devices[0]?.deviceTemplate?.frontPanelUrl} alt={device.name} />
    </div>
  );
};
