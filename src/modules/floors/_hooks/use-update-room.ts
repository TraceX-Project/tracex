...
export const useUpdateRoom = () => {
  ...
  return useMutation({
    mutationFn: ({ roomId, floorId, payload }: { roomId: string, floorId: string, payload: UpdateRoomInput }) => updateRoom(roomId, payload),
    onMutate: async ({ roomId, floorId, payload }) => {
      await Promise.all([
        queryClient.cancelQueries({ queryKey: [QUERY_KEYS.rooms, roomId] }),
      ])
    ...
    onError: (err, vars, context) => {
      if (context?.prevRoomList) {
        queryClient.setQueryData<Room[]>(
          [QUERY_KEYS.rooms, vars.floorId],
          context.prevRoomList
        )
      }
    },
    onSettled: (data) => {
